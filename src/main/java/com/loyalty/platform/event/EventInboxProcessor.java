...
public class EventInboxProcessor {
    ...
    /**
     * 从标准 payload 中提取业务事件时间。
     * 优先使用请求体中的业务时间戳（如 trade.pay_time），降级使用当前时间。
     */
    private Instant extractEventTime(Map<String, Object> payload) {
        if (payload == null) return Instant.now();

        // 1. 顶层 pay_time（天猫、京东等电商回调）
        Object payTime = payload.get("pay_time");
        if (payTime != null) {
            try {
                return parseToInstant(String.valueOf(payTime));
            } catch (Exception ignored) {}
        }

        // 2. 顶层 event_time
        Object eventTime = payload.get("event_time");
        if (eventTime != null) {
            try {
                return parseToInstant(String.valueOf(eventTime));
            } catch (Exception ignored) {}
        }

        // 3. 嵌套 trade.pay_time (天猫会员通特有结构)
        Object tradeObj = payload.get("trade");
        if (tradeObj instanceof Map) {
            Object nestedPayTime = ((Map<?, ?>) tradeObj).get("pay_time");
            if (nestedPayTime != null) {
                try {
                    return parseToInstant(String.valueOf(nestedPayTime));
                } catch (Exception ignored) {}
            }
        }

        // 4. Unix timestamp (秒或毫秒)
        Object ts = payload.get("timestamp");
        if (ts instanceof Number) {
            long val = ((Number) ts).longValue();
    ...
    /**
     * 解析常见的时间字符串为 Instant。
     * 支持 ISO-8601、yyyy-MM-dd HH:mm:ss、yyyy-MM-dd'T'HH:mm:ss 等格式。
     */
    private Instant parseToInstant(String timeStr) {
        if (timeStr == null || timeStr.isBlank()) throw new IllegalArgumentException("empty");
        ...
        // ISO-8601 / LocalDateTime 格式
        try {
            return Instant.parse(trimmed);
        } catch (Exception ignored) {}

        // yyyy-MM-dd HH:mm:ss（天猫常见格式）
        try {
        ...
        throw new IllegalArgumentException("Unparseable time: " + timeStr);
    }
}