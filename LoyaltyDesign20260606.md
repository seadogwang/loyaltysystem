...
```
#### 2.2.1.1 Kafka/EventBridge 顺序性约束
**背景**：Kafka 保证每个 Partition 内消息有序，但跨 Partition 则无序。**务必将同一会员的所有消息发送至同一分区**。
* **Partition Key 规则**：固定使用 `member_id` 作为 PartitionKey，确保同会员消息一贯路由。如：`kafkaTemplate.send(topic, memberId, event)`。
* **消费端并发限制**：每个分区只能由一个线程处理以保证顺序。Spring Kafka 默认对每个分区使用单线程。**禁止开启多个消费者实例同时消费同一Topic的多个分区，否则相当于并行处理单会员消息**。如需提升并发，可增加分区数与消费者实例，但保证每会员的分区Key不变。
...
### 2.4 AI 开发脚手架契约：统一包结构与核心基类
为保证AI编码时的规范一致性，本小节定义强制性的包结构、核心过滤器及引擎实现模板。所有新增代码必须严格遵循以下契约。
#### 2.4.1 系统统一包结构定义
```text
com.loyalty.platform
├── common
│   ├── context      // 租户多租户上下文 (TenantContext)
│   ├── crypto       // PII 加解密与 BlindIndex 工具
│   └── exception    // 统一异常与错误码定义
├── domain
│   ├── member
│   │   ├── entity   // Member, MemberUniqueKey
│   │   ├── repository
│   │   └── service  // SagaMemberMergeService
│   ├── points
│   │   ├── entity   // AccountTransaction, MemberAccount
│   │   └── engine   // FifoOffsetEngine (游标加速版)
│   ├── rules
│   │   ├── engine   // KieBaseCacheManager (Drools 8 隔离)
│   │   └── model    // Fact, EventFact, MemberFact
│   └── spi
│       ├── gateway  // SpiGatewayController
│       └── handler  // ChannelSpiHandler 策略模式实现类
└── infrastructure
    ├── eventbus     // Dev/Prod 分流的 EventBridge 实现
    └── security     // DataMaskingAspect (切面脱敏)
```
#### 2.4.2 强隔离的数据防穿透拦截器基类
所有 Repository 层查询必须通过以下 `TenantContextFilter` 强制注入租户上下文，禁止在业务代码中手动拼接 `program_code` 过滤。
```java
...
public class TenantContextFilter implements Filter {
    private static final String TENANT_HEADER = "X-Program-Code";
    @Override
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
        ...
        if (tenantCode == null || tenantCode.trim().isEmpty()) {
            log.error("安全防御阻断：缺失租户上下文Header X-Program-Code");
            throw new IllegalArgumentException("SECURITY_ERR: Tenant code is missing");
        }
        try {
            TenantContext.set(tenantCode);
            chain.doFilter(request, response);
        } finally {
            TenantContext.clear(); // 绝不向下一个请求残留
        }
    }
}
```
#### 2.4.3 高并发 FIFO 游标扣减引擎实现契约
`FifoOffsetEngine` 必须使用乐观锁 + 状态段游标加速方案，严禁扫描零偏移量的全部历史账单。
```java
package com.loyalty.platform.domain.points.engine;
import com.loyalty.platform.common.exception.BusinessException;
import com.loyalty.platform.domain.points.entity.AccountTransaction;
...
@Service
public class FifoOffsetEngine {
    @Transactional
    public void deductPointsFifo(String tenantId, String memberId, String accountType, BigDecimal pointsToRedeem) {
        BigDecimal remaining = pointsToRedeem;
        
        // 1. 获取游标 (Optimized Cursor)
        Long lastCursorId = getOrCreateCursor(tenantId, memberId, accountType);
        
        while (remaining.compareTo(BigDecimal.ZERO) > 0) {
            // 2. 批量拉取活跃批次（避免大偏移量，每次读取30条）
            List<AccountTransaction> batches = fetchActiveBatchesAfter(
                tenantId, memberId, accountType, lastCursorId, 30
            );
            
            if (batches.isEmpty()) {
                break; // 积分余额不足，跳出后由外层触发信用/透支逻辑
            }
            ...
            for (AccountTransaction batch : batches) {
                ...
                // 4. 扣减资产并更新状态
                BigDecimal deductAmount = remaining.min(batch.getRemainingAmount());
                batch.setRemainingAmount(batch.getRemainingAmount().subtract(deductAmount));
                
                if (batch.getRemainingAmount().compareTo(BigDecimal.ZERO) == 0) {
                    batch.setStatus("EXHAUSTED");
                    lastCursorId = batch.getId(); // 推进游标
                }
                
                saveBatch(batch);
                createRedemptionAllocation(batch, deductAmount);
                remaining = remaining.subtract(deductAmount);
            }
            
            // 5. 持久化最新游标位置
            updateCursor(tenantId, memberId, accountType, lastCursorId);
        }
        
        if (remaining.compareTo(BigDecimal.ZERO) > 0) {
            // 剩余未扣完，尝试执行被动透支或主动信用授信
            handleOverdraftOrCredit(tenantId, memberId, remaining);
        }
    }
    private List<AccountTransaction> fetchActiveBatchesAfter(String tenant, String member, String type, Long cursorId, int size) {
        // SQL 映射语义: SELECT * FROM account_transaction 
        // WHERE program_code = :tenant AND member_id = :member AND account_type = :type AND id > :cursorId 
        // AND status = 'ACTIVE' AND remaining_amount > 0 AND expires_at > NOW() ORDER BY expires_at ASC, id ASC LIMIT :size
        return null; // AI 待落地具体ORM查询
    }
    
    private Long getOrCreateCursor(String tenant, String member, String type) { return 0L; }
    private void updateCursor(String tenant, String member, String type, Long cursorId) {}
    private void saveBatch(AccountTransaction tx) {}
    private void createRedemptionAllocation(AccountTransaction source, BigDecimal amount) {}
    private void handleOverdraftOrCredit(String tenant, String member, BigDecimal remaining) {}
}
```
***
...
入会请求到达 → 提取所有可用标识
↓
【新增】规范化洗涤 (Normalization)：
    对明文手机号：去除非数字字符（空格、+86、-），仅保留纯数字
    例如 "+86 138-1234-5678" → "13812345678"
↓
第一层：强标识精确匹配（使用经过规范化的明文手机或原始渠道ID）
...
注：加密前也必须对明文手机进行规范化洗涤
→ 命中 → 绑定，写入 MOBILE_PLAIN（规范化后的值）
→ 未命中 → 进入第三层
↓
第三层：创建新 member_id
→ 写入所有可用标识到 member_unique_key（手机号存入前再次规范化）
→ 对于抖音用户，标记为"待验证"状态
↓
异步补充验证（仅限抖音）：
→ 用户下单时抖音返回完整手机号
→ 规范化洗涤后写入 MOBILE_PLAIN
→ 触发跨渠道匹配检查 → 自动合并
```
...
public void processEnrollment(EnrollmentRequest request) {
    ...
    String plainMobileRaw = request.getPlainMobile(); // 明文手机（如有）
    
    // 【新增】规范化洗涤：去除非数字字符
    String plainMobile = normalizePhoneNumber(plainMobileRaw);
...
private String normalizePhoneNumber(String raw) {
    ...
    return raw.replaceAll("[^0-9]", "");  // 仅保留数字
}
```
#### 3.4.6 数据库唯一约束兜底
```java
...
### 3.5 Member Merge Saga（会员合并编排）
**背景**：自动合并需跨多个实体同步积分、等级、优惠券、标识等，涉及长事务。引入 Saga 编排保证安全性。
> **注意**：自动合并操作**仅**创建合并任务，实际资产迁移通过异步 Saga 执行。禁止在唯一约束冲突捕获或同步入会流程中直接执行会员全量合并，以避免长事务锁表和数据不一致。
**目的与风险说明**：会员合并操作涉及多张表的并发读写，风险极高，一旦处理不当可能导致积分丢失、重复、等级错误等问题。采用 Saga 模式将合并过程分解为多个可补偿步骤，中间状态可持久化，支持失败回滚和审计。并通过账户冻结避免并发消费引发不一致。
#### 3.5.1 Saga 状态机
```meraid
stateDiagram-v2
    [*] --> CREATED
    CREATED --> FREEZING : 开始合并
    FREEZING --> TRANSFER_POINTS : 冻结账户
    TRANSFER_POINTS --> TRANSFER_TIERS : 转移积分批次
    TRANSFER_TIERS --> TRANSFER_COUPONS : 转移优惠券
    TRANSFER_COUPONS --> TRANSFER_TAGS : 转移标签
    TRANSFER_TAGS --> TRANSFER_IDENTITIES : 转移身份
    TRANSFER_IDENTITIES --> REBUILD_CACHE : 重建缓存
    REBUILD_CACHE --> COMPLETED : 完成
    FREEZING --> FAILED
    TRANSFER_POINTS --> FAILED
    TRANSFER_TIERS --> FAILED
    TRANSFER_COUPONS --> FAILED
    TRANSFER_TAGS --> FAILED
    TRANSFER_IDENTITIES --> FAILED
    REBUILD_CACHE --> FAILED
    FAILED --> ROLLBACK : 触发补偿
    ROLLBACK --> [*]
```
* **CREATED**：收到合并请求，生成合并任务记录 (member_merge_task)，状态 `CREATED`。
* **FREEZING**：标记合并双方账户为 `MERGING`（冻结），以禁止积分变动。见下文 “冻结机制”。
* **TRANSFER_POINTS**：按 FIFO 保留顺序将源会员的所有“有效积分流水批次”归属到目标会员（更新 `account_transaction.member_id` 并维护 `remaining_amount`）。见下文“积分迁移”。
* **TRANSFER_TIERS**：合并等级：取两者最高等级，记录合并前后异动到 `tier_change_log`。
* **TRANSFER_COUPONS**：迁移所有 **未使用且未过期** 的优惠券，将 `coupon_instance.member_id` 更新为目标会员。
* **TRANSFER_TAGS**：合并会员标签集（取并集，不重复）。
* **TRANSFER_IDENTITIES**：合并身份标识（如手机号、第三方账号）。**必须采用软删除重定向机制处理强标识冲突**（见 3.5.8 节修正）。
* **REBUILD_CACHE**：清除并重建相关缓存，发布 `MemberMergedEvent`。
* **COMPLETED**：Saga 成功结束。
补偿路径：若任一步骤失败，Saga 进入 `FAILED`，然后倒序执行补偿（依次从身份、标签、优惠券、等级、积分逆向恢复），最后回滚冻结（标记账户为正常），并记录失败状态。
#### 3.5.2 合并任务表
新增表 `member_merge_task` 存储 Saga 状态：
```sql
CREATE TABLE member_merge_task (
    ...
    status VARCHAR(32) NOT NULL,         -- Saga 状态: CREATED, FREEZING, ..., COMPLETED, FAILED, ROLLBACK
    current_step VARCHAR(64),            -- 当前执行步骤
    error_message TEXT,
    created_at TIMESTAMPTZ DEFAULT NOW(),
    updated_at TIMESTAMPTZ DEFAULT NOW(),
    UNIQUE (program_code, source_member_id) -- 同一会员只能有一个活跃合并任务
);
```
* **program_code**：租户隔离。
* **Unique 约束**：防止重复创建相同源会员的并发合并任务。
#### 3.5.3 冻结机制
新增账户状态字段，禁止冻结期操作：
```sql
ALTER TABLE member_account ADD COLUMN account_status VARCHAR(16) DEFAULT 'ACTIVE';
```
* 状态 `MERGING` 表示冻结中。合并任务开始时，将源和目标会员的所有 `member_account` 行状态更新为 `MERGING`。
* 在 `MemberAccountService` 等所有修改积分的入口，加校验：
  java
  
  ```
  if (memberAccount.getAccountStatus().equals("MERGING")) {
      throw new BusinessException("ERR_MEMBER_MERGING", "会员正在合并，操作禁止");
  }
  ```
* 冻结期间禁止积分入账、核销、透支、调整等操作，确保流水稳定。
合并完成或失败时，将状态改回 `ACTIVE`。
#### 3.5.4 积分资产迁移
**禁止**直接修改余额字段，应通过转移流水记录来完成。示例：
```java
@Transactional
public void transferPoints(String programCode, Long sourceId, Long targetId) {
    // 1. 归属转移：解冻模式不允许直接更新余额
    List<AccountTransaction> batches = txRepo.findActiveBatchesForUpdate(sourceId, accountType);
    for (AccountTransaction batch : batches) {
        batch.setMemberId(targetId);
        txRepo.save(batch);
    }
    // 2. 更新账户累计统计（自增）
    MemberAccount srcAcct = accountRepo.findForUpdate(sourceId, accountType);
    MemberAccount tgtAcct = accountRepo.findForUpdate(targetId, accountType);
    tgtAcct.setTotalAccrued(tgtAcct.getTotalAccrued().add(srcAcct.getTotalAccrued()));
    tgtAcct.setTotalRedeemed(tgtAcct.getTotalRedeemed().add(srcAcct.getTotalRedeemed()));
    accountRepo.save(tgtAcct);
}
```
**要点**：每个积分批次（`account_transaction`）保留原始 `expires_at`、`rule_id` 等，审核合并前后的 `remaining_amount` 不变。仅改变 `member_id`，避免重新计算。
#### 3.5.5 等级迁移
遵循“高等级保留”原则：
```java
Member src = memberRepo.findById(sourceId);
Member tgt = memberRepo.findById(targetId);
String newTier = maxTier(src.getTierCode(), tgt.getTierCode()); // 取更高等级
if (!newTier.equals(tgt.getTierCode())) {
    // 记录降级/升级日志
    tierLogRepo.save(new TierChangeLog(targetId, tgt.getTierCode(), newTier, "MERGE"));
    tgt.setTierCode(newTier);
    memberRepo.save(tgt);
}
```
保留被合并会员的最高等级，删除冗余等级历史后续可在后台归档或合并。
#### 3.5.6 优惠券迁移
```sql
UPDATE coupon_instance
SET member_id = :targetId
WHERE member_id = :sourceId
  AND status = 'AVAILABLE' 
  AND expires_at > NOW()
  AND program_code = :programCode;
```
仅迁移 **未使用且未过期** 的券，已使用或已过期的券留在原记录中；若商家需要，可在审核完后启用自定义补发流程。
#### 3.5.7 标签合并
```java
Set<String> srcTags = tagRepo.findTagsByMember(sourceId);
Set<String> tgtTags = tagRepo.findTagsByMember(targetId);
Set<String> union = new HashSet<>(srcTags);
union.addAll(tgtTags);
tagRepo.saveAll(memberId=targetId, tags=union);
tagRepo.deleteByMember(sourceId);
```
简单并集逻辑，已存在的标签不重复存储。所有标签修改通过事件发送至权限校验和缓存模块。
#### 3.5.8 身份标识迁移（含软删除重定向与防死锁修正）
**【修正】强标识冲突处理与分布式加锁排序**
合并时迁移 `member_unique_key` 表项。遵循以下规则：
* **强标识（手机号、邮箱）**：若冲突（两个会员绑定同手机号），**不能直接插入或标记无效**，因为 UNIQUE 索引会直接抛出异常。必须采用**软删除重定向**：
  1. 在事务内将源会员的冲突标识的 `key_value` 修改为 `原值:merged:{timestamp}`（例如 `13812345678:merged:20260606120000`），释放 UNIQUE 索引占位。
  2. 然后向 `member_unique_key` 中插入一条指向目标会员的新记录，`key_value` 为原始值。
  3. 若目标会员已存在该标识，则跳过源会员的该标识迁移。
* **弱标识（渠道 openId 等）**：直接附加到目标会员，冲突时保留目标会员的，源标识同样进行软删除重定向。
**分布式死锁预防**：在 SAGA 开始阶段，获取两个会员的分布式锁时必须按 member_id 自然顺序排序，防止并发互锁。
```java
// 伪代码：在合并入口处
List<Long> memberIds = Arrays.asList(sourceId, targetId);
memberIds.sort(Long::compareTo);  // 升序排序
RLock lock1 = redissonClient.getLock("member:merge:" + memberIds.get(0));
RLock lock2 = redissonClient.getLock("member:merge:" + memberIds.get(1));
lock1.lock();
lock2.lock();
try {
    // 执行合并逻辑
...
迁移具体实现：
```java
// TRANSFER_IDENTITIES 步骤中的关键代码
List<UniqueKey> keys = uniqueKeyRepo.findAllByMember(sourceId);
for (UniqueKey key : keys) {
    // 先软删除源标识，释放唯一约束
    String oldValue = key.getKeyValue();
    String newValue = oldValue + ":merged:" + System.currentTimeMillis();
    key.setKeyValue(newValue);
    uniqueKeyRepo.save(key);
    
    try {
        // 重新插入指向目标会员的原始标识（若目标会员还没有该标识）
        uniqueKeyRepo.save(new UniqueKey(
            programCode, key.getKeyType(), oldValue, targetId, 
            key.isStrong(), key.isVerified()
        ));
    } catch (DataIntegrityViolationException ex) {
        // 目标会员已存在该标识，无需处理，记录审计日志
        auditLog.warn("强标识冲突已通过软删除解决: {} 原属于 {}，现属于 {}", oldValue, sourceId, targetId);
    }
}
```
#### 3.5.9 缓存重建与事件
所有合并数据持久化后，清理缓存并发出合并事件：
```java
memberCache.evict(sourceId);
memberCache.evict(targetId);
eventBridge.publish("loyalty-member-events", programCode,
    new MemberMergedEvent(sourceId, targetId, programCode, ZonedDateTime.now()));
```
事件 `MemberMergedEvent` 包含 `sourceMemberId` 和 `targetMemberId`，供各业务引擎刷新状态。
#### 3.5.10 补偿机制
如果 Saga 中某步失败，应逆序补偿前面成功的步骤，并最终失败记录保存：
```java
@Transactional
public void compensateMerge(long taskId) {
    MergeTask task = taskRepo.findById(taskId);
    task.setStatus("ROLLBACK");
    task.setCurrentStep("COMPENSATING");
    taskRepo.save(task);
    
    // 示例：逆序恢复优惠券归属
    List<Coupon> coupons = couponRepo.findByMember(task.getTargetId());
    for(Coupon c: coupons) {
        if (c.getMergeTaskId() == taskId) {
            c.setMemberId(task.getSourceId());
            couponRepo.save(c);
        }
    }
    // 依次补偿标签、等级、积分流水等
    // 最后恢复冻结状态
    accountRepo.findByMemberId(task.getTargetId())
               .forEach(ac -> { ac.setAccountStatus("ACTIVE"); accountRepo.save(ac); });
    accountRepo.findByMemberId(task.getSourceId())
               .forEach(ac -> { ac.setAccountStatus("ACTIVE"); accountRepo.save(ac); });
    ...
    // 审计和告警
    auditLog.error("会员合并任务 {} 失败并补偿", taskId);
}
```
任何自动执行失败导致的残留问题应生成运维告警、人工核查日志。
#### 3.5.11 审计日志
新增审计表 `member_merge_audit` 记录合并全程关键数据（操作人、时间、合并前后积分和券数量、结果等）。所有合并动作（手动或自动）应写入。示例结构：
```sql
...
```
> **注意：** 如前所述，**自动合并仅在 MemberMergeTask** 中异步执行，禁止在唯一约束异常或注册流程中同步合并，以避免长事务和死锁。
***
## 第四章：积分、等级与资产隔离域
### 4.1 积分类型字典
...
```
#### 4.2.3 透支熔断机制（防零元爆刷修正）
**【新增】被动透支触发后立即熔断兑换接口**
当任何操作导致账户产生被动透支流水（`transaction_type = 'OVERDRAFT'`）且未被清偿时，除了将账户状态标记为 `SUSPENDED_REDEMPTION`，还必须在账户快照表 `member_account_snapshot` 中增加 `frozen_status` 控制字段，并在全局兑换入口进行实时拦截。
```sql
ALTER TABLE member_account ADD COLUMN frozen_status VARCHAR(16) DEFAULT 'ACTIVE';
-- 可选值: ACTIVE, FROZEN_REDEMPTION, FROZEN_ALL
```
在 `redeemPoints` 方法的最开头加入校验：
```java
@Transactional
public void redeemPoints(String programCode, String memberId, String accountType, BigDecimal pointsToRedeem) {
    ...
    if ("FROZEN_REDEMPTION".equals(account.getFrozenStatus()) || "FROZEN_ALL".equals(account.getFrozenStatus())) {
        throw new BusinessException("ERR_ACCOUNT_FROZEN", "账户因透支已被冻结兑换权限，请先归还欠款");
    }
    // 原有逻辑 ...
}
```
当被动透支发生并超出 `overdraft_limit` 时，设置 `frozen_status = 'FROZEN_REDEMPTION'`，并生成追偿工单。
#### 4.2.4 冲抵引擎约束
...
* **设置 `frozen_status = 'FROZEN_REDEMPTION'`**（见 4.2.3 节）
...
#### 7.3.2 FIFO 大规模优化（游标方案）
详见第二章 2.4.3 节 `FifoOffsetEngine` 实现契约。核心要点：
* 维护 `member_fifo_cursor` 表记录每会员上次消费到的流水ID
* 分批查询 `id > lastCursorId` 的活跃批次，避免全表扫描
* 使用 `FOR UPDATE SKIP LOCKED` 支持并发安全
### 7.4 事件标准化生命周期与死信处理
...
API 请求 → SpiGatewayController 接收
  ...
  → One-ID 匹配（member_unique_key 查询，含规范化洗涤）
  → 规则引擎推理（Drools DRL）
...
```
### 7.6 数据流映射可视化表达
为帮助开发人员和 AI 理解数据加工流向，以下流程图概括了从异构请求到积分发放的全过程：
```text
  [异构三方入会请求 (如天猫 OUID/抖音 Mask)]
                   │
                   ▼ (SPI Webhook Gateway 验签)
        [Unified SPI Gateway]
                   │
         ┌─────────┴─────────┐
         ▼ (VISUAL 模式)     ▼ (SCRIPT 模式)
   [JSONPath 配置映射]     [GraalVM JS 隔离沙箱]
   (e.g., tradeNo->id)     (e.g., mapOrderType())
         └─────────┬─────────┘
                   ▼
         [TransactionEvent 标准化]
                   │
                   ▼ (HMAC-SHA256 清洗并计算 Blind Index)
       [One-ID 匹配 (辅助唯一键表)]
                   │
         ┌─────────┴─────────┐
         ▼ (标识匹配成功)     ▼ (标识匹配失败)
    [复用 member_id]      [新建 member_id]
         └─────────┬─────────┘
                   ▼
       [Drools 8 规则引擎匹配] (KieBase 动态载入)
                   │
                   ▼
        [PointGrant 瀑布冲抵] (更新快照与事务流)
```
***
## 第八章：管理界面设计与前端规范
### 8.1 总体思路与前端设计哲学
...
### 9.2 四层绝对防御体系
**第一层：入口防御**：`TenantFilter` 强制要求所有请求携带 `program_code`（参见 2.4.2 节 `TenantContextFilter`）。
...
### 10.3.1 幂等中心 (Idempotent Center)
**背景**：仅靠 Redis 缓存易丢失，需持久化记录所有幂等操作。
**数据结构**：
```sql
CREATE TABLE idempotent_record (
    tenant_id VARCHAR(32) NOT NULL,
    idempotency_key VARCHAR(128) NOT NULL,
    request_hash VARCHAR(64),       -- 请求内容哈希（可选）
    response_data JSONB,
...
```
* `status` 初始插入为 `IN_PROGRESS`，完成后更新为 `COMPLETED` 并保存结果。
* 业务处理失败时删除记录或置失败标志，允许重试。
* `expires_at` 设置过期时间（如24小时），过期后可清理记录。
**处理流程**（伪码）：
```java
public ApiResponse handleRequest(String tenant, String idemKey, byte[] payload) {
    Optional<IdempotentRecord> recOpt = repo.findByTenantAndKey(tenant, idemKey);
    if (recOpt.isPresent()) {
        IdempotentRecord rec = recOpt.get();
        if (rec.status == COMPLETED) {
            return rec.responseData; // 直接返回上次结果
        } else {
            throw new BusinessException("IDEMPOTENCY_IN_PROGRESS", "请求正在处理中");
        }
    }
    // 插入新记录
    IdempotentRecord rec = new IdempotentRecord(tenant, idemKey, hash(payload), IN_PROGRESS, now());
    repo.save(rec);
    try {
        ApiResponse result = processBusiness(payload);
        rec.setResponseData(result);
...
```
* 关键点：利用唯一主键约束保证幂等键全局唯一；插入失败则直接认定为重复请求。
  **Redis加速**：可选将幂等键缓存在 Redis，减少 DB 命中。命中后直接返回结果。
  **注意**：Redis 丢失时仍能依赖 DB 保证一致。
### 10.4 错误处理规范
禁止返回 HTTP 500。业务失败返回 HTTP 200，Body 中包含业务错误码。
***
## 第十一章：数据库物理模型设计
（与 v7.0 基本相同，但需补充 `member_account` 表的 `frozen_status` 字段以及 `member_fifo_cursor` 表。下面仅列出新增/修改部分，完整表结构请参考 v7.0。）
```sql
-- 1. 核心租户计划表
...
***
## 第十二章：AI 单元测试套件规范
为防止 AI 在开发过程中发生逻辑漂移，编写规则时必须强制引入测试回归框架。以下是高并发场景下的单元测试基座。
```java
package com.loyalty.platform;
import com.loyalty.platform.common.context.TenantContext;
import com.loyalty.platform.domain.points.engine.FifoOffsetEngine;
...
@SpringBootTest
public class ConcurrencyLoyaltyIntegrationTest {
    @Autowired
    private FifoOffsetEngine fifoEngine;
    private final String TENANT = "BRAND-A";
    private final String MEMBER_ID = "MEM-TEST-999";
    @BeforeEach
    public void setup() {
        TenantContext.set(TENANT);
        // 初始化会员测试账户、授信额度以及初始活跃积分批次
        initializeTestAccount(TENANT, MEMBER_ID);
    }
    @Test
    public void testHighConcurrentDeduction() throws InterruptedException {
        int threadCount = 10;
        ExecutorService executor = Executors.newFixedThreadPool(threadCount);
        CountDownLatch latch = new CountDownLatch(1);
        CountDownLatch finishLatch = new CountDownLatch(threadCount);
        // 模拟并发 10 个渠道同时对同一会员发起 50 积分扣减（总可用 400，应透支或引发信用扣减）
        for (int i = 0; i < threadCount; i++) {
            executor.submit(() -> {
                try {
                    latch.await(); // 阻断所有线程，确保绝对并发
                    TenantContext.set(TENANT);
                    fifoEngine.deductPointsFifo(TENANT, MEMBER_ID, "REWARD", new BigDecimal("50"));
                } catch (Exception e) {
                    // 记录并发异常
                } finally {
                    TenantContext.clear();
        ...
        // 验证并发后的总账数据准确性
        BigDecimal finalBalance = getRemainingPointsSum(TENANT, MEMBER_ID, "REWARD");
        // 原 400 扣减 500，扣减后可用余额应为 0，且信用积分应当扣除 100
    ...
    private void initializeTestAccount(String t, String m) {}
    private BigDecimal getRemainingPointsSum(String t, String m, String type) { return BigDecimal.ZERO; }
}
```
所有核心业务模块（积分发放、核销、会员合并、级联重算）必须提供对应的单元测试，覆盖正常路径、并发场景及异常补偿路径。
***
## 附录：系统字典与枚举值汇总
### A.1 会员相关
#### A.1.1 会员状态 (member.status)
| 值             | 说明         |
| ------------- | ---------- |
| `ENROLLED`    | 正常注册       |
| `SUSPENDED`   | 已停用        |
...
***
### A.2 渠道相关
#### A.2.1 渠道类型 (channel / channelType)
| 值        | 说明   |
| -------- | ---- |
| `TMALL`  | 天猫   |
| `JD`     | 京东   |
...
| `VIP`    | 唯品会  |
| `POS`    | 线下门店 |
#### A.2.2 渠道标识类型 (member_unique_key.key_type)
| 值                    | 说明          | is_strong |
| -------------------- | ----------- | ---------- |
| `MOBILE_PLAIN`       | 明文手机        | true       |
| `TMALL_OUID`         | 天猫唯一ID      | true       |
| `JD_PIN`             | 京东唯一ID      | true       |
| `WECHAT_OPENID`      | 微信openId    | true       |
| `WECHAT_UNIONID`     | 微信unionId   | true       |
| `DOUYIN_OPENID`      | 抖音用户ID      | true       |
| `TMALL_MOBILE_MD5`   | 天猫双重MD5密文手机 | false      |
| `JD_MOBILE_ENCRYPT`  | 京东密文手机      | false      |
| `DOUYIN_MOBILE_MASK` | 抖音掩码手机      | false      |
#### A.2.3 映射模式 (channel_adapter_config.mapping_mode)
| 值        | 说明              |
| -------- | --------------- |
| `VISUAL` | 可视化连线映射         |
| `SCRIPT` | GraalVM JS 脚本映射 |
#### A.2.4 渠道适配器状态 (channel_adapter_config.status)
| 值        | 说明  |
| -------- | --- |
| `DRAFT`  | 草稿  |
| `ACTIVE` | 已启用 |
***
### A.3 交易相关
#### A.3.1 事件类型 (TransactionEvent.event_type)
| 值               | 说明    |
| --------------- | ----- |
| `ORDER`         | 正单交易  |
| `ORDER_REFUND`  | 退单交易  |
...
***
### A.4 积分相关
#### A.4.1 积分类型代码 (account_type)
| 值        | 名称    | 说明                 |
| -------- | ----- | ------------------ |
| `REWARD` | 消费积分  | 用户交易累积，可兑换礼品       |
| `TIER`   | 等级成长值 | 只用于计算会员等级，不可消费     |
| `CREDIT` | 授信积分  | 系统授予的信用额度，可透支使用需归还 |
#### A.4.2 交易流水类型 (account_transaction.transaction_type)
| 值                 | 说明            |
| ----------------- | ------------- |
| `ACCRUAL`         | 积分发放（正向入账）    |
| `REDEMPTION`      | 积分核销（兑换扣除）    |
| `EXPIRATION`      | 积分过期          |
| `REPAYMENT`       | 透支还款          |
| `CREDIT_REPAY`    | 信用还款          |
| `CREDIT_DRAWDOWN` | 信用提取（使用信用额度）  |
| `OVERDRAFT`       | 被动透支（退款导致的负数） |
#### A.4.3 流水状态 (account_transaction.status)
| 值           | 说明        |
| ----------- | --------- |
| `ACTIVE`    | 有效（有剩余额度） |
| `EXHAUSTED` | 已耗尽       |
...
| 配置项               | 类型      | 说明       |
| ----------------- | ------- | -------- |
| `is_redeemable`   | Boolean | 可否用于兑换   |
| `is_tier_calc`    | Boolean | 是否计入等级计算 |
| `is_transferable` | Boolean | 可否转赠     |
| `allow_negative`  | Boolean | 是否允许余额为负 |
#### A.4.5 有效期模式
| 值               | 说明   |
| --------------- | ---- |
| `FIXED_DAYS`    | 固定天数 |
...
***
### A.5 等级相关
#### A.5.1 等级变更原因 (tier_change_log.change_reason)
| 值                   | 说明             |
| ------------------- | -------------- |
| `UPGRADE`           | 升级             |
| `DOWNGRADE`         | 降级（定时保级降级）     |
| `CASCADE_DOWNGRADE` | 级联降级（退款引发的降级）  |
| `MERGE`             | 合并（会员合并时取最高等级） |
***
### A.6 事件处理相关
#### A.6.1 event_inbox 状态 (event_inbox.status)
| 值                  | 说明                 |
| ------------------ | ------------------ |
...
| `PROCESSING`       | 执行映射引擎转换           |
| `COMPLETED`        | 处理完成               |
| `REJECTED`         | 验签失败 / 幂等重复 / 字段缺失 |
| `TRANSFORM_FAILED` | 映射/转换失败            |
| `RETRYING`         | 自动重试中              |
| `DEAD`             | 重试耗尽，转入死信队列        |
#### A.6.2 拒绝原因 (event_inbox.reject_reason)
| 值                | 说明     |
| ---------------- | ------ |
| `DUPLICATE`      | 幂等重复   |
...
***
### A.8 Schema 相关
#### A.8.1 Schema 状态 (program_schema.status)
| 值            | 说明          |
| ------------ | ----------- |
| `DRAFT`      | 草稿          |
| `ACTIVE`     | 已发布生效       |
| `DEPRECATED` | 已废弃（被新版本替代） |
#### A.8.2 实体类型 (dataflow 设计器)
| 值          | 说明     |
| ---------- | ------ |
| `SYSTEM`   | 系统实体   |
| `BUSINESS` | 业务实体   |
...
| `PATH`      | 路径映射                |
| `SCRIPT`    | 脚本映射                |
| `CONTAINER` | 容器映射（业务实体存入JSONB容器） |
#### A.8.5 字段类型 (FieldType)
| 值         | 说明         |
| --------- | ---------- |
| `String`  | 字符串        |
| `Number`  | 数字         |
...
| `Array`   | 数组         |
| `Ref`     | 引用（引用其他实体） |
#### A.8.6 Formily 组件类型 (x-component)
| 值              | 说明    |
| -------------- | ----- |
| `Input`        | 输入框   |
| `NumberPicker` | 数字选择器 |
...
***
### A.10 级联重算相关
#### A.10.1 级联重算任务状态 (cascade_recalc_job.status)
| 值            | 说明           |
| ------------ | ------------ |
| `PENDING`    | 待处理          |
| `PROCESSING` | 正在执行         |
...
### A.x 账户冻结状态 (member_account.frozen_status)
| 值                   | 说明             |
| ------------------- | -------------- |
| `ACTIVE`            | 正常             |
| `FROZEN_REDEMPTION` | 冻结兑换（可入账不可兑换）  |
| `FROZEN_ALL`        | 完全冻结（不可入账不可兑换） |
其它枚举值保持不变。
***
## 附录：系统错误码汇总
### B.1 通用错误码
...
| ------------------ | ------ |
| `SUCCESS`          | 操作成功   |
| `ERR_BAD_REQUEST`  | 参数校验错误 |
| `ERR_UNAUTHORIZED` | 认证失效   |
| `ERR_FORBIDDEN`    | 权限不足   |
| `ERR_RATE_LIMITED` | 请求频率过高 |
### B.2 业务错误码
| 错误码                               | 说明           |
| --------------------------------- | ------------ |
| `ERR_INSUFFICIENT_POINTS`         | 积分不足         |
| `ERR_FIELD_IN_USE`                | 字段被规则引用，无法废弃 |
| `ERR_INVALID_STATUS`              | 状态不合法        |
| `ERR_UNKNOWN_ACTION`              | 未知操作         |
| `ERR_MOBILE_NOT_MATCHED`          | 手机号验证失败      |
| `ERR_VALIDATION`                  | 联合校验失败       |
| `RULE_CONFLICT_REQUIRES_OVERRIDE` | 规则冲突需强制放行    |
| `ERR_TIMEOUT`                     | SPI 处理超时     |
| `ERR_MEMBER_MERGING` | 会员正在合并，操作禁止 |
| `ERR_ACCOUNT_FROZEN` | 账户已被冻结      |
***
**文档结束**
